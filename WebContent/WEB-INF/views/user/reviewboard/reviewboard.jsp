...
<small>클래스를 수강한 오디분들이 작성해주신 포토 후기 게시판입니다.</small>
</div>
<hr>
...
<!-- 후기가 없을 때  -->
<c:if  var="review" test="${empty reviewboard }">
...
<c:if  var="review" test="${!empty reviewboard }">
<!-- 값 출력 -->
<c:forEach items="${reviewboard }" var="review" >
    ...
    <div class="thumbnail boxstyle"> 
     <img src="/upload/${review.reviewrename }" alt="포토후기 사진" class="imgst">
      <div class="caption">
        ...
        <small>${review.usernick }</small>&nbsp;<small>${review.usergrade }</small>&nbsp;<small>${review.reviewdate }</small>
        <p class="contentwrap">${review.reviewcontent }</p>
        <hr>
        <small>[${review.artnick }]</small>&nbsp;<small class="contentwrap">${review.classname }</small>
