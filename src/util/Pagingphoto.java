...
public class Pagingphoto {
	...
	//총 게시글 수만 입력하는 생성자
	 public Pagingphoto(int totalCount) {
	 ...
	 public Pagingphoto(int totalCount,int curPage) {
		 setTotalCount(totalCount);
		 setCurPage(curPage);
	 ...
	 public Pagingphoto(int totalCount,int curPage,int listCount) {
		 setTotalCount(totalCount);
		 setCurPage(curPage);
	 ...
	 public Pagingphoto(int totalCount,int curPage,int listCount, int pageCount) {
		 setTotalCount(totalCount);
		 setCurPage(curPage);
		 setListCount(listCount);
	 ...
	 private void makePaging() {
		 ...
		 if(listCount == 0 ) setListCount(9); //화면에 보여질 게시글 수 기본값
		 System.out.println(pageCount);
		 if(pageCount == 0 ) setPageCount(9); //화면에 보여질 페이지수 기본 값
